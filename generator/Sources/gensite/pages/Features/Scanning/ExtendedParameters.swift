...
struct ExtendedParameters: View {
  var body: some View {
    Page(
      ...
      description: "Extending Pelican with custom parameters.",
      keywords: [
        "OBD-II",
        "car scanner",
        "trip logger",
          ...
          VStack(alignment: .center) {
            ...
            Div {
              H1("Extended Parameters")
                ...
                .bold()
                .fontDesign("rounded")
              Text("How to extend Pelican's functionality with custom parameters")
            }
...
### Extended PIDs in Pelican

In addition to supporting many of the industry standard OBD-II PIDs specified in [SAE J1979](https://www.sae.org/standards/content/j1979da_202404/), Pelican pulls extended PIDs from the OBDb, [github.com/OBDb](https://github.com/OBDb).
...
When you connect a scanner to your car, Pelican will ask for the car's VIN in order to determine its make and model. The make and model will then be used to enable the relevant extended PIDs within the app.
...
Contributions from the automotive community help expand the library of extended PIDs available in Pelican. By sharing extended PIDs, we all contribute to the ability for car owners to access more of their vehicle data.
...
Connectables are elements of the Pelican user interface that can be powered by values returned from an OBD-II scanner. Connectables expect their values to be provided in a certain [Unit](#unit).
...
\(inlineHTML { Parameter("path", type: "String") }) The navigation path for the signal in Pelican's parameters page. If not provided, the signal's id property will be used as the navigation path instead.<br/>
...
If a unit includes a version (e.g. **v1.13+**), that means the unit was introduced in that version of the Pelican application. Older app versions will fall back to treating unknown units as a scalar.
